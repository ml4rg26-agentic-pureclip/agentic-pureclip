...
#   bash scripts/download_data.sh --clip-only       # only CLIP BAMs
#   bash scripts/download_data.sh --motifs-only     # only motif databases
#   bash scripts/download_data.sh --dry-run         # show what would be downloaded
#
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            echo "Usage: bash scripts/download_data.sh [--clip-only] [--motifs-only] [--dry-run]"
            exit 0
            ;;
    esac
done
...
download_motifs() {
    ...
    uv run python scripts/list_motifs.py || true
}
...
echo ""
echo "Next steps:"
echo "  1. Verify: python scripts/list_motifs.py"
echo "  2. Index BAMs: find data/ -name '*.bam' | while read b; do samtools index \"\$b\"; done"
echo "  3. Run a dataset: snakemake -s workflow/Snakefile -j 8 --configfile config/datasets/RBFOX2_K562.yaml"
