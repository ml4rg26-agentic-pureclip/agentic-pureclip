...
Launch (on the server, inside tmux so it survives disconnects):
    ...
    PYTHONPATH=. uv run python scripts/overnight_batch.py --hours 8

Dry run (plan only, no compute):
    PYTHONPATH=. uv run python scripts/overnight_batch.py --dry-run
"""
...
from pipeline.datasets import dataset_to_config  # noqa: E402
from pipeline.configs import sample_bams  # noqa: E402
from agent.decisions import composite_objective, DEFAULT_OBJECTIVE_WEIGHTS  # noqa: E402
