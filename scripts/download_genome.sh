...
#
# Usage:
#   bash scripts/download_genome.sh              # full primary assembly (~900 MB)
#   bash scripts/download_genome.sh --chr21      # chr21 only for quick testing (~2 MB)
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            echo "Usage: bash scripts/download_genome.sh [--chr21]"
            echo ""
...
echo "  snakemake -s workflow/Snakefile -j 8 --configfile config/datasets/RBFOX2_K562.yaml"
echo "=================================================="
