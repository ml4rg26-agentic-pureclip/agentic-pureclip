...
Usage:
    uv run python scripts/run/batch_runner.py                    # run all
    uv run python scripts/run/batch_runner.py --ids quick_*      # run matching IDs
    uv run python scripts/run/batch_runner.py --dry-run          # show what would run
    uv run python scripts/run/batch_runner.py --parallel 2       # run 2 at a time
...
def execute_run(run: dict) -> dict:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "agentic_pureclip.loop.graph"],
        env=env,
        capture_output=True,
        text=True,
    )
    ...
    # Parse best score from output. agentic_pureclip.loop.graph ends with
