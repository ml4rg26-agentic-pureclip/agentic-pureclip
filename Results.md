...
are given so each change is unambiguous. **Update (post-merge `a7a7e39`, biology-
grounded prompt):** anchors re-checked against `develop`. **Change 1 (multi-param
moves) is now already satisfied** by the rewritten prompt (see below — kept as-is),
and Change 2's strip-list is expanded because the prompt is biology-coupled well
beyond the JSON block.

---

> **Implementation status (branch `final-results-repo-changes`):** Changes 2, 3, 4,
> arm-tagging and the batch manifest are **implemented + tested** (one commit each;
> full suite green, 40 passed / 1 skipped). Change 1 needed no code (see below). The
> runs have **not** been launched. Outstanding: Part 3 analysis/plot scripts (post-run).
...
**Four remaining items:** Changes 2–4 below, plus the small **arm-tagging** addition
(detailed under *Storage & provenance*). Change 1 (multi-param moves) is already
done — satisfied by the current prompt. All appear in the change-summary table at
the end.

### Change 1 — Multi-parameter LLM moves (fairness: applies to all results)  [DONE — no code change]
**Status:** already satisfied by the biology-grounded prompt (commit `a7a7e39`).
Optuna samples all 7 params every trial, so the LLM must be free to change several
at once for a fair comparison. The current `build_decision_prompt`
(`scoring/objective.py`) **decision rule #3** already reads:

> *"Prefer changing ONE parameter at a time so its effect stays interpretable, but
...
This is the intended behavior and is kept as-is: it gives the LLM a light,
generally-sound optimization/interpretability nudge while explicitly permitting
multi-parameter moves. `apply_decision_changes` (`pipeline/configs.py`) already
applies every key in `changes`, and `tunable_signature` dedups full multi-key
snapshots — so multi-param moves already work end-to-end. **No code change and no
test change needed** (no test asserts on the "ONE parameter" wording). The
`reasoning` field still explains the move, so the reasoning slide is unaffected.
...
- `scoring/objective.py` → `build_decision_prompt`: add a param
  `include_priors: bool = True`. **The prompt is now deeply biology-grounded
  (commit `a7a7e39`), so "no priors" means stripping more than the JSON block.**
  When `False`:
  - replace the `PRIOR KNOWLEDGE` JSON block with a withheld-placeholder line (prior
    fields are `target_protein`, `expected_footprint_width_nt`,
    `preferred_transcript_region` / `preferred_binding`, `known_motifs`,
    `binding_breadth` — re-read the live keys rather than trusting this list; they
    drifted between `priors.json` and the dataset YAMLs);
  - drop the biology framing from the opening paragraph ("the RNA-binding protein
    described in PRIOR KNOWLEDGE… use what is known about its binding biology");
  - drop the whole **"HOW THE BIOLOGY SHOULD INFORM YOUR CHOICES"** section;
  - neutralize **decision rule #1** ("Ground each choice in this protein's binding
    biology…") to a generic deltas-only version;
  - relax the response-format line ("reflect on this RBP's role and
    preferred_binding region…") so it doesn't demand naming the protein.
  Keep the weights line and the `SEARCH BOUNDS` block (shared game, not a prior).
...
1. **Multi-param scope:** allow **multiple simultaneous changes**. The current
   biology-grounded prompt (rule #3) gives a light interpretability nudge ("prefer
   one at a time, but you MAY change 2-3 together… avoid changing many at once")
   while permitting multi-param moves. **Kept as-is** — the guidance is generally
   sound optimization advice and does not block the LLM from changing several
   parameters. Realized in `a7a7e39`; no further change (see Change 1).
...
|---|---|---|
| 1 Multi-param LLM | *(none — satisfied by prompt rule #3 in `a7a7e39`)* | Done |
