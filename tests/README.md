...
- **`test_agent.py`**: Contains tests for the LangGraph state machine.
  ...
  - `test_real_gemini_api`: An integration test that uses the DeepSeek API to run a 2-iteration loop. This test requires a valid API key in the `.env` file.
...
To run the real DeepSeek API integration test:
```bash
